...
엄은상 개인 홈페이지 (이력 관리용). 정적 HTML + JS, 빌드 도구 없음.
...
assets/app.js       cv.js → HTML 렌더링, EN/KO 토글
...
`data/cv.js` 열어서 해당 배열에 객체 추가/수정. 텍스트는 `{ en: "...", ko: "..." }` 형태.

| 할 일 | 수정 위치 |
|---|---|
| 논문 accept | `publications` 맨 위에 추가 + `news` 맨 위에 한 줄 |
...
| CV PDF | `files/CV_Eunsang_Eom.pdf` 에 파일 넣기. 다른 이름이면 `meta.cvPdf` 수정 |
